...
class PerformanceAttributionAnalyzer:
    ...
    def _calculate_factor_attribution(
        ...
        for factor in factor_returns_aligned.columns:
            ...
            portfolio_returns = (weights_aligned * stock_returns_aligned).sum(axis=1)
    ...
    def _calculate_sector_attribution(
        ...
        for ticker in weights_aligned.columns:
    ...
    def _calculate_stock_selection_attribution(
        ...
        if sector_mapping:
            ...
            for sector in set(sector_mapping.values()):
                ...
                sector_weights = weights_aligned[[t for t in sector_tickers if t in weights_aligned.columns]]
                sector_returns_data = stock_returns_aligned[[t for t in sector_tickers if t in stock_returns_aligned.columns]]
            ...
            benchmark_returns = stock_returns_aligned.mean(axis=1)
    ...
    def _calculate_timing_attribution(
        ...
        initial_weights = weights_aligned.iloc[0]
        buy_hold_returns = (initial_weights * stock_returns_aligned).sum(axis=1)
        buy_hold_total = buy_hold_returns.sum()
        
        # Calculate rebalanced return
        rebalanced_returns = (weights_aligned * stock_returns_aligned).sum(axis=1)
